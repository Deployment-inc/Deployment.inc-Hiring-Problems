...
3. Complete the [private submission form](https://deployment.inc/hiring-challenges/#submit) with your name, email, phone, chosen problem, private repository URL and a few lines on your approach. Attach `submission.yaml` following [SUBMISSION_SCHEMA.md](SUBMISSION_SCHEMA.md); it carries the **exact commit SHA** we evaluate, and its problem and repository must match the form. Do not post any submission metadata, code, experiment logs or results in public issues, discussions or PRs.
...
To resubmit, send the updated YAML (with the new commit SHA) through the form, then email hiring@deployment.inc with your old and new submission IDs so we can replace the active entry. Updating the repository alone does not change the evaluated version. One active submission per person per problem. Email the same address with your submission ID to withdraw at any time.
